    ...
    <div className="flex flex-col md:flex-row gap-6">
      {qr && (
        <div className="shrink-0 text-center">
          <img src={qr} alt="订阅二维码" className="w-[160px] h-[160px] rounded-xl border mx-auto" style={{ borderColor: 'var(--color-line)' }} />
        ...
        {rows.map(([k, v]) => (
          <div key={k}>
            ...
            <div className="flex gap-2 items-center">
              ...
              <button type="button" className="btn-ghost h-9 shrink-0" onClick={() => copy(v)}><Icon name="copy" size={14} /> 复制</button>
            </div>
          </div>
        ))}
        <div className="flex flex-wrap gap-2 pt-1">
          <a className="btn-ghost h-9" href={clashImportURL(token)}>打开 Clash</a>
          <a className="btn-ghost h-9" href={singboxImportURL(token)}>打开 sing-box</a>
          <button type="button" className="btn-ghost h-9" onClick={() => copy(auto, '已复制自动识别链接')}><Icon name="link" size={14} /> 复制自动识别</button>
        </div>
