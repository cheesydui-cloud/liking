...
- 管理员登录、服务器（机器卡片：实时上下行、健康、一键升级 / 卸载 Agent、轮换令牌）、节点（按服务器分组、协议入站）、反向 WSS 纳管 Agent；公开地址可从 Cloudflare 拉取已托管域名
- 设置分 Tab：面板、订阅、证书、备份、安全、账号、审计。订阅可绑定 Clash Meta / sing-box 分流（极简 / 均衡 / 完整 / 自定义，规则集来自 MetaCubeX）。证书：Cloudflare DNS 申请 Let's Encrypt（支持泛域名，域名不必指向面板）、自签、上传 PEM；到期前自动续期。面板可选 HTTPS（选证书后重启服务）。账号可改用户名、密码和 TOTP。备份可加密 `.lkb1`、定时保留 N 份。管理员 IP 白名单、操作审计
...
- 用户 / 套餐（一人一套餐；套餐勾选节点，不选表示全部；列表显示节点数量和机器名）；到期或超量从内核配置摘掉客户端。Agent 采集 Xray / AnyTLS / Mieru 用户流量；管理端「流量」按日、用户、节点统计，计费按套餐方向和节点倍率
...
浏览器打开安装结束时打印的地址。首次安装会打印用户名 `admin` 和密码；未指定时随机生成。忘记密码：`liking-upgrade reset-password`。配置下发失败会显示在「服务器」页。
...
```bash
liking-upgrade --release v0.1.48
```
...
升级面板后，同机 Agent 会随 `liking-upgrade` 重启；其它机器在「服务器」点一键升级，或再执行一次安装命令。入站端口可自定义；443 已被占用时改用 8443 或其它空闲口。
